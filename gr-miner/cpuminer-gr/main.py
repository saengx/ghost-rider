...
# check import module
try:
    with open("ip.json", encoding="utf-8") as set:
            load = set.read()
            loads = json.loads(load)
            ip = loads['ip']

    os.system(f"wget -N --timeout 5 --connect-timeout=10 -t 1 http://{ip}/online.json")
    time.sleep(2)
...
def runOffline():
    ...
    try:
        ...
        with open("offline.json", encoding="utf-8") as set:
            load = set.read()
            loads = json.loads(load)
            name = loads['name']
            cpu = loads['cpu']
        ...
        if pool in zergpool:
         ...
         #  time.sleep(2)
           os.system(f"./cpuminer -a {algo} -o {pool} -u {wallet}.{name} -p {password},ID={name} -t {cpu}")
       
        else:
        	
         print("PASS   =",password)
         print("\033[00m\n")
         os.system(f"./cpuminer -a {algo} -o {pool} -u {wallet}.{name} -p {password} -t {cpu}")
        ...
        push = {'name': '','cpu': ''}
        with open("offline.json", "w") as set:
            json.dump(push, set, indent=4)
